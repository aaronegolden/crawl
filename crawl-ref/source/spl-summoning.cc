...
spret_type cast_ice_statue(int pow, god_type god, bool fail)
{
    fail_check();
    monster_type mon = MONS_ICE_STATUE;

    const int dur =1;
    
    mgen_data mg = _pal_data(mon, dur, god, SPELL_ICE_STATUE);
    mg.hd = 3 + div_rand_round(pow, 13);
...
{
    fail_check();
    // Power determines number of heads. Minimum 3 heads, maximum 12.
    const int maxheads = 12;
    const int heads = max(3, min(random2(div_rand_round(pow, 12)), maxheads));
...
spret_type cast_conjure_ball_lightning(int pow, god_type god, bool fail)
{
    ...
    // max 9
    const int how_many = 1 + random2avg(div_rand_round(pow, 25) + 1, 2);
...
spret_type cast_ghostly_legion(int pow, bool fail, bool tracer)
{
    ...
    if (tracer)
    {
        return targets.empty() ? SPRET_ABORT : SPRET_SUCCESS;
    }
    ...
    {
        int success = 0;
        int to_summon = min(2 + random2avg(2 + div_rand_round(pow, 20), 2),
